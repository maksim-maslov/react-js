...
class DroppedMenu extends Component {
  ...
  render() {
    ...
    return(
      <div className={`dropped-menu ${droppedMenuVisible ? 'dropped-menu_visible' : ''}`} onMouseLeave={() => updateDroppedMenuVisible(-1, false)}>
        <div className="wrapper">
          ...
          <div className="dropped-menu__lists dropped-menu__lists_three-coloumns">
            ...
            <ul className="dropped-menu__list">
              {type.map((el, index) => <li key={index} className="dropped-menu__item"><a>{el}</a></li>)}
            </ul>
          </div>
          <div className="dropped-menu__lists">
            <h3 className="dropped-menu__list-title">Сезон</h3>
            <ul className="dropped-menu__list">
              {season.map((el, index) => <li key={index} className="dropped-menu__item"><a>{el}</a></li>)}
            </ul>
          </div>
          <div className="dropped-menu__lists">
            <h3 className="dropped-menu__list-title">Бренды</h3>
            <ul className="dropped-menu__list">
              {brands.map((el, index) => <li key={index} className="dropped-menu__item"><a>{el}</a></li>)}             
            </ul>
          </div>
        </div>
      </div>
